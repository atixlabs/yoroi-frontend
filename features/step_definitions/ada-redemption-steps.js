...
Given(/^I have accepted "Daedalus Redemption Disclaimer"$/, async function () {
  await this.client.execute(() => {
    window.yoroi.actions.ada.adaRedemption.acceptRedemptionDisclaimer.trigger();
  });
});

Given(/^I am on the ada redemption screen$/, async function () {
  await navigateTo.call(this, '/ada-redemption');
  return this.waitForVisible('.AdaRedemptionForm_component');
});

Given(/^I see the "Daedalus Redemption Disclaimer" overlay$/, function () {
  return this.waitForVisible('.AdaRedemptionDisclaimer_component');
});
...
Then(/^I should not see the "Daedalus Redemption Disclaimer" overlay anymore$/, function () {
  return this.waitForVisible('.AdaRedemptionDisclaimer_component', null, true);
});
...
When(/^I enter a valid "Regular" redemption key$/, function () {
  ...
  return this.setValue('.AdaRedemptionForm_component .redemption-key input', redemptionKey);
});

When(/^I select a valid "Regular" PDF certificate$/, async function () {
  await this.chooseFile(CERTIFICATE_UPLOAD_BOX, regularAdaCertificateFilePath);
});

When(/^I select a valid "Regular" encrypted PDF certificate$/, async function () {
  await this.chooseFile(CERTIFICATE_UPLOAD_BOX, regularEncryptedAdaCertificateFilePath);
});
...
When(/^I enter a valid "Regular" encrypted PDF certificate passphrase$/, async function () {
  ...
  for (let i = 0; i < passphrase.length; i++) {
    ...
    await this.setValue('.AdaRedemptionForm_component .pass-phrase input', word);
    await this.waitForVisible(`//li[contains(text(), '${word}')]`);
    await this.waitAndClick(`//li[contains(text(), '${word}')]`);
    await this.waitForVisible(`//span[contains(text(), '${word}')]`);
  }
});
...
When(/^I select a valid "Force vended" PDF certificate$/, async function () {
  await this.chooseFile(CERTIFICATE_UPLOAD_BOX, forceVendedAdaCertificateFilePath);
});

When(/^I select a valid "Force vended" encrypted PDF certificate$/, async function () {
  await this.chooseFile(CERTIFICATE_UPLOAD_BOX, forceVendedEncryptedAdaCertificateFilePath);
});
...
When(/^I enter a valid "Force vended" encrypted PDF certificate email, passcode and amount$/, async function () {
  ...
  const amount = '12345';
  await this.setValue('.AdaRedemptionForm_component .email input', email);
  await this.setValue('.AdaRedemptionForm_component .ada-passcode input', passcode);
  await this.setValue('.AdaRedemptionForm_component .ada-amount input', amount);
});

When(/^I enter a valid "Force vended" encrypted PDF certificate decryption key "([^"]*)"$/, async function (decryptionKey) {
  await this.setValue('.AdaRedemptionForm_component .decryption-key input', decryptionKey);
});

When(/^I enter a valid "Paper vended" shielded vending key$/, function () {
  return this.setValue('.AdaRedemptionForm_component .shielded-redemption-key input', '6ANn43jbzR7zZGnV3BYnna1myW5HajPgjiCPg4vpcayf');
});
...
When(/^ada redemption form submit button is no longer disabled$/, function () {
  return this.waitForEnabled(REDEMPTION_SUBMIT_BUTTON);
});
...
Then(/^I should see the "Ada Redemption Success Overlay"$/, function () {
  return this.waitForVisible('.AdaRedemptionSuccessOverlay_component');
});
